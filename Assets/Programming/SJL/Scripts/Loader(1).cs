...
public class Loader1 : MonoBehaviour
{
    ...
    public void Start()
    {
        ...
        //confirmButton.gameObject.SetActive(false);  // 확인 버튼 비활성화
        //StartCoroutine(LoadSceneCoroutine("JYL_MainScene"));
    }

    //IEnumerator LoadSceneCoroutine(string sceneName)    // 씬 로딩 코루틴
    //{
    ...
    //    {
    //        // asyncOp.progress는 0 ~ 0.9까지 올라가고
    //        // allowSceneActivation = true 해야 1.0이 됨
    //        float progress = Mathf.Clamp01(operation.progress / 0.9f);

    //        if (percentSlider != null)
    //            percentSlider.value = progress;
    //        if (percentText != null)
    //            percentText.text = "로딩 " + (progress * 100f).ToString("F0") + "%";  

    //        // 거의 다 로드 됐으면 확인 버튼 활성화
    //        if (progress >= 1f && confirmButton != null && !confirmButton.gameObject.activeSelf)
    ...
    public void Update()
    {
        if (loading)
        {
            ...
            if (progress < 1f)
                progress += Time.deltaTime / 2f; // 2초간 0→1
